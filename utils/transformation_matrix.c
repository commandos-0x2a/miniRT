...
t_mat4  *ft_get_scale_mat(t_object *obj)
{
	t_mat4 *scale_mat;
	
	scale_mat = (t_mat4 *)malloc(sizeof(t_mat4));
	if (!scale_mat)
	{
		//log_error("Failed to allocate memory for scale matrix");
		return (NULL);
	}
...
t_mat4 	*ft_get_rotation_mat(t_object *obj)
{
	t_mat4 *rotation_mat;
	t_rcx rcx;
	
	rotation_mat = (t_mat4 *)malloc(sizeof(t_mat4));
	if (!rotation_mat)
	{
		//log_error("Failed to allocate memory for rotation matrix");
		return (NULL);
	}
