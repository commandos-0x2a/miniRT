...
/*   Updated: 2025/07/31 19:49:56 by yaltayeh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_triangle(mlx_image_t *image, t_vector2 tex_ver[3])
{
	t_vector2 p[3] = {
		{0.f, 0.f},
	...
	for (float y = sy; y <= ey; y++)
	{
		for (float x = sx; x <= ex; x++)
		{
			...
			mlx_put_pixel(image, (uint32_t)x, (uint32_t)y, ft_pixel(lambda.x, lambda.y, lambda.z, 1));
		}
	}
}
...
int32_t main(void)
{
	...
	t_vector2 verties[3] = {
		{0.f, 0.f},
		{(float)image->width, 0.f},
		{0.f, (float)image->height}
	};
	draw_triangle(image, verties);
	mlx_loop(mlx);
	mlx_terminate(mlx);
	return (EXIT_SUCCESS);
}
